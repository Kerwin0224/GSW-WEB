...
-- 空间功能：云端零残留验证
--
-- 用法：整段粘进 Supabase Studio 的 SQL Editor，一次性执行。
--       最后一句是 rollback，所以**不会留下任何东西**——Postgres 的 DDL 是事务性的。
--
-- 看什么：
--   · 执行过程零 ERROR  = 迁移能在真实 schema 上跑通
--   · NOTICE 里的自检   = 结构、函数修饰符、策略谓词、触发器都在位
--   · 最后两张「anon可执行」的表格 = KEEP 全 t、REVOKED 全 f
--     这一条最关键：revoke 清单里漏判任何一个应用层在用的函数，线上对应功能当场 42501
--
-- 为什么在这里验而不是本地：本项目全程直连云端，不起本地栈；而且只有真实 schema
-- 才跑得通迁移自带的全部自检（本地桩缺函数，自检起不来）。
--
-- 出错怎么办：自检失败会 raise exception，事务随即进入**中止态**，末尾那句 rollback 不会
-- 自动执行。这时手动再跑一句 `rollback;` 即可（中止态下只有 rollback 能被接受）。
-- 中止态不等于「改坏了」——aborted 事务的所有变更本来就不会落盘。
--
-- 验证完直接关掉这个标签页即可，不需要任何清理动作。
-- ══════════════════════════════════════════════════════════════════════════════

begin;

-- ┌──────────────────────────────────────────────────────────────────────────┐
-- │ 迁移 1／2：20260916152531_spaces_and_membership.sql                       │
-- └──────────────────────────────────────────────────────────────────────────┘
...
-- ┌──────────────────────────────────────────────────────────────────────────┐
-- │ 迁移 2／2：20260916160943_fix_classes_insert_and_anon_execute.sql         │
-- └──────────────────────────────────────────────────────────────────────────┘
...
-- ══════════════════════════════════════════════════════════════════════════════
-- 探针：自检之外的可见结论（只读，不改任何东西）
-- ══════════════════════════════════════════════════════════════════════════════

-- 1) 该保留 anon EXECUTE 的函数——必须全是 t。
--    任何一个 f 都意味着线上对应功能当场 42501。
select 'KEEP' as 分组, p.proname as 函数,
       has_function_privilege('anon', p.oid, 'execute') as anon可执行
 ...
 order by anon可执行, p.proname;

-- 2) 该收回的函数——必须全是 f。
select 'REVOKED' as 分组, p.proname as 函数,
       has_function_privilege('anon', p.oid, 'execute') as anon可执行
  from pg_proc p join pg_namespace n on n.oid = p.pronamespace
 where n.nspname = 'public'
   and p.proname in (
     'refresh_project_highest_bloom_level','get_profile','project_catalog_path',
     'set_initial_password_by_login','rls_auto_enable','authenticate_school_account',
     'authenticate_school_account_v2','authenticate_user','rebuild_scenario_provider_capabilities',
     'clear_school_model_tier_binding','sync_project_contract','validate_conversation_contract',
     'validate_space_contract'
   )
 order by anon可执行 desc, p.proname;

-- 3) classes 的两条策略：WITH CHECK 不能再出现按 id 回查表的 can_admin_class(id)
select policyname as 策略名, cmd as 命令, coalesce(with_check::text, '(无)') as 写侧条件
  from pg_policies where schemaname = 'public' and tablename = 'classes'
 order by policyname;

-- 4) spaces 的策略：应恰好 3 条，且**没有** delete
select policyname as 策略名, cmd as 命令
  from pg_policies where schemaname = 'public' and tablename = 'spaces'
 order by cmd, policyname;

-- 5) 还有多少班级没归属学校（这些人拉不进空间，需要人工补）
select count(*) as 无学校归属的班级数 from public.classes where school_id is null;
