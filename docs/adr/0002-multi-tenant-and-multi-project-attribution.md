...
多租户先行（独立迁移链），群文阅读其次（依赖归类器，改动面在应用层更多）。两者互不阻塞，各自走"迁移 → 云端验证（`db push --dry-run --linked` + 策略探针）→ 预览 → merge"标准流程。（原文写的是 `db reset`，那是本地栈时代的步骤；本仓已改为纯云端开发，见 `docs/agents/deployment.md` 硬性约束。）
