...
export async function POST(req: Request) {
  return withApiLogging(req, { area: 'api', event: 'challenge_evaluate', route: '/api/challenge/evaluate' }, async () => {
    ...
    if (!capability.data.ready) return Response.json({ state: 'blocked', error: 'Practice evaluation provider not configured', resolution: capability.data.blockedReason }, { status: 503 });

    const model = resolveLanguageModel(capability.data);
    if (!model) return Response.json({ state: 'blocked', error: 'Practice evaluation model unavailable', resolution: `${capability.data.providerName ?? 'Provider'} 的 secret_ref 未在服务端环境中解析成功。` }, { status: 503 });
    ...
    try {
      const result = await generateObject({
        ...
        prompt: `你是古诗文挑战评估助手。请根据题目目标层级、学生作答和篇目信息，判断学生是否达成该布鲁姆层级。必须严格评估，不要因为有回答就判定通过。\n篇目：《${project?.title ?? '未知篇目'}》${project?.author ? `，作者：${project.author}` : ''}\n目标层级：L${practice.target_bloom_level}\n挑战题：${practice.prompt}\n学生作答：${parsed.data.answer}\n输出要求：achieved 表示是否达成；feedback 给学生具体、可执行的学习反馈。`,
      });
      evaluation = result.object;
    } catch (error) {
      await supabase.from('practice_records').update({ answer: parsed.data.answer, evaluation_state: 'failed', feedback: error instanceof Error ? `评估调用失败：${error.message}` : '评估调用失败：Provider 返回未知错误。' }).eq('id', practice.id);
      return Response.json({ state: 'failed', error: '真实挑战评估调用失败。', resolution: error instanceof Error ? error.message : 'Provider 返回未知错误。' }, { status: 502 });
    }
    ...
    const { data: updatedPractice, error: updateError } = await supabase
      ...
      .select('*')
      .single();
    if (updateError) return Response.json({ state: 'error', error: `评估结果保存失败：${updateError.message}` }, { status: 500 });
