...
export default async function AdminDashboard() {
  ...
  const hasTraceRecords = logStatus.appLogBytes > 0 || logEvents.length > 0 || exports.length > 0;
        ...
        <Card className={studentMissing.length ? 'border-destructive/35 bg-destructive/6' : 'border-primary/25 bg-primary/6'}>
          <CardHeader>
            <CardTitle className="flex items-center justify-between gap-3">
              ...
              <Badge variant={studentMissing.length ? 'destructive' : 'secondary'}>{studentMissing.length ? '路由缺失' : '路由已配置'}</Badge>
            </CardTitle>
          </CardHeader>
        ...
        <Card className={teacherMissing.length ? 'border-destructive/35 bg-destructive/6' : 'border-primary/25 bg-primary/6'}>
          <CardHeader>
            <CardTitle className="flex items-center justify-between gap-3">
              ...
              <Badge variant={teacherMissing.length ? 'destructive' : 'secondary'}>{teacherMissing.length ? '路由缺失' : '路由已配置'}</Badge>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3 text-sm">
            <p className="leading-6 text-muted-foreground">
