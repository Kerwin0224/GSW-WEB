...
05-05 UIUX 主线已经落地学生、教师、管理员三角色核心页面，但仍有两类可见体验与 `CONTEXT.md` 的产品语境不完全一致：教师侧仍暴露“审计”等后台治理术语，管理员侧虽然已有用户与班级数据入口雏形，但缺少独立的用户权限页与班级成员管理入口。
...
- [ ] 教师路由与教师组件可见中文文案不再出现“审计”“SFT”“DPO”“打标”。
- [ ] 教师学情页面使用“本周核实覆盖”“待核实学习记录”等领域语言。
- [ ] 管理员侧边栏学校管理分组包含“用户权限”。
- [ ] `/admin/users` 展示 profiles 角色、状态、账号信息，并提供 CSV 导入入口。
- [ ] `/admin/classes` 展示每个班级的教师/学生成员概览，并提供成员管理入口。
- [ ] 管理员 AI 运维与教学数据导出仍可显示 SFT/DPO/Provider/MCP 专业术语。
- [ ] `npm --prefix web run lint` 通过。
- [ ] `web/node_modules/.bin/tsc --project web/tsconfig.json --noEmit --pretty false` 通过。
