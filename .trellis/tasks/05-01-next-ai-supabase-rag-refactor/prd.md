# Classical Chinese AI Workbench — Stack & Spec Lock + Product PRD Convergence

## Goal

Lock the target technical stack and executable implementation specs for the next version of the Classical Chinese AI Workbench, then use Trellis to converge the product PRD from the user's rough PRD direction.

This task is primarily a planning/spec task before implementation. It should not start a full rewrite until product scope and acceptance criteria are narrowed.
...
The target stack is fixed as:

- **Application framework**: Next.js App Router on Vercel
- **AI runtime**: Vercel AI SDK for streaming responses, provider abstraction, tool calling, and embeddings
- **Database/Auth/Storage/Realtime**: Supabase
- **Vector retrieval**: Supabase Postgres + pgvector
- **Orchestration layer**: no mandatory LangChain.js/LlamaIndex.TS dependency for MVP. Use Vercel AI SDK primitives plus a narrow internal orchestration/retrieval adapter. Add LangChain.js or LlamaIndex.TS only if concrete workflow complexity justifies it.
- **Type/validation layer**: TypeScript + generated Supabase database types + Zod request validation

## Product Direction Sources

The product direction should be derived from these documents, with Trellis responsible for turning them into a consistent detailed PRD:

1. [`PRD`](../../../PRD) — user's rough product vision. It contains errors but captures the broad idea: a school-facing classical poetry/classical Chinese AI teaching workbench with student, teacher, admin, learning interactions, audit, and dataset production.
2. [`docs/prd_decision_v1.md`](../../../docs/prd_decision_v1.md) — narrower four-module product reset: login, student, teacher, admin. Treat as a useful scoping correction unless the user overrides it.
3. [`docs/demo_scope_v1.md`](../../../docs/demo_scope_v1.md) — demo spine for proving the narrowed product flows.
4. [`research/next-ai-supabase-rag-best-practices.md`](research/next-ai-supabase-rag-best-practices.md) — external best-practice research for the locked stack.
...
- Email must be syntactically valid and unique within the import batch.
...
- Use the rough product PRD as directional input, but let Trellis correct contradictions and scope creep.
...
None at this level. Ready for final PRD confirmation or implementation-slice planning.
...
- Deleting unrelated legacy FastAPI/SQLite/Chroma backend files outside the assigned frontend/Next/Supabase change scope.
- Finalizing UI design details before product scope is converged.
- Treating the rough `PRD` document as error-free.
