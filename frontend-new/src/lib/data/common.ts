...
export type CapabilityStatus = { capability: ProviderCapability; ready: boolean; modelId?: string; providerName?: string; providerType?: string; baseUrl?: string | null; blockedReason?: string };
...
export async function getCapability(capability: ProviderCapability): Promise<DataResult<CapabilityStatus>> {
  const envReady = capability !== 'embedding' && (process.env.OPENAI_API_KEY || process.env.AI_GATEWAY_API_KEY) && process.env.DEFAULT_CHAT_MODEL;
  if (envReady) return ok({ capability, ready: true, modelId: process.env.DEFAULT_CHAT_MODEL, providerName: process.env.AI_GATEWAY_API_KEY ? 'server-env-gateway' : 'server-env-openai', providerType: process.env.AI_GATEWAY_API_KEY ? 'gateway' : 'openai' });

  try {
    const supabase = await createClient();
    const { data, error } = await supabase
      .from('provider_capabilities')
      .select('capability,model_id,is_enabled,provider_configs(name,is_enabled,health_status,provider_type,base_url)')
    ...
    if (provider.health_status === 'failed' || provider.health_status === 'blocked') return ok({ capability, ready: false, modelId: data.model_id, providerName: provider.name, providerType: provider.provider_type, baseUrl: provider.base_url, blockedReason: `${provider.name} 健康状态为 ${provider.health_status}。` });
    return ok({ capability, ready: true, modelId: data.model_id, providerName: provider.name, providerType: provider.provider_type, baseUrl: provider.base_url });
  } catch (error) {
