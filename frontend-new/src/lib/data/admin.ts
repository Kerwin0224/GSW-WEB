...
export async function saveProviderConfig(formData: FormData): Promise<void> {
  ...
  if (!name || !provider_type || !model_id || selected.length === 0) return;
  const supabase = await createClient();
  const { data: provider, error } = await supabase.from('provider_configs').insert({ name, provider_type, base_url, secret_ref: apiKey ? `env:${name.toUpperCase().replace(/\W+/g, '_')}_API_KEY` : null, secret_last_four: apiKey ? apiKey.slice(-4) : null, is_enabled: Boolean(apiKey), health_status: apiKey ? 'unchecked' : 'blocked', created_by: role.data.id }).select('id').single();
  if (error) return;
  const { error: capError } = await supabase.from('provider_capabilities').insert(selected.map((capability) => ({ provider_id: provider.id, capability, model_id, is_enabled: Boolean(apiKey) })));
  if (capError) return;
